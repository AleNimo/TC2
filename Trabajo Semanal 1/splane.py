...
def analyze_sys( all_sys, start_frec_exponent = -2, stop_frec_exponent = 6, sys_name = None, img_ext = 'none', same_figs=True ):
    ...
    for ii in range(cant_sys):
        fig_id, axes_hdl = bodePlot(all_sys[ii], start_frec_exponent, stop_frec_exponent, fig_id, axes_hdl, label = sys_name[ii])
...
def bodePlot(myFilter, start_frec_exponent, stop_frec_exponent, fig_id='none', axes_hdl='none', label = '', npoints = 1000 ):
    ...
    if isinstance(myFilter, np.ndarray):
        ...
        for ii in range(cant_sos):
            ...
            num, den = one_sos2tf(myFilter[ii,:])
            thisFilter = TransferFunction(num, den)
            w, mag[:, ii], phase[:,ii] = thisFilter.bode(np.logspace(start_frec_exponent,stop_frec_exponent,npoints))
            sos_label += [label + ' - SOS {:d}'.format(ii)]
        
        # whole filter
        thisFilter = sos2tf_analog(myFilter)
        w, mag[:, cant_sos], phase[:,cant_sos] = thisFilter.bode(np.logspace(start_frec_exponent,stop_frec_exponent,npoints))
        sos_label += [label]
        ...
        # LTI object
        cant_sos = 0
        w, mag, phase = myFilter.bode(np.logspace(start_frec_exponent,stop_frec_exponent,npoints))
